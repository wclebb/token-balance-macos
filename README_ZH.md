...
- 显示 5 小时和 Weekly 剩余额度百分比，菜单分别列出自然重置时间和倒计时。
...
下拉菜单分别显示两个窗口的剩余百分比、本地重置日期和时间、倒计时，以及可用重置卡数量和最近到期时间。额外 Reset 临近过期时，菜单栏追加提醒（例如 ` · ⚠ R 8h`），保留两个百分比和可用重置卡数量。未提供的额度窗口显示 `—%`，不会用 5 小时额度代替 Weekly，也不会用百分比伪造调用次数。上游提供的 Weekly 精确剩余次数仍显示在菜单中。
